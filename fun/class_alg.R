dm <- function(file_path_cods, file_paths_bd, file_path_literatura) {
  #códigos para DM
  cods <- read_excel(file_path_cods, sheet = "DM")
  ...
  #Bases de datos
  bd2011 <- read_excel(file_paths_bd, sheet = "2011")
  names(bd2011) <- epitrix::clean_labels(names(bd2011))
  bd2011 <- bd2011 %>% mutate(name_drug = epitrix::clean_labels(med))

  bd2011$enf_cie10 <- NA
  bd2011$enf_cie10[bd2011$cod_cie10 %in% cods_cie10$codigo] <- 1
  bd2011$enf_cups <- NA
  bd2011$enf_cups[bd2011$cod_cie10 %in% cods_cups$codigo] <- 1
  bd2011$enf_cups[bd2011$med %in% names_cups] <- 1
  library(stringr)
  ...
  bd2012 <- read_excel(file_paths_bd, sheet = "2012")
  names(bd2012) <- epitrix::clean_labels(names(bd2012))
  bd20122_1 <- strsplit(bd2012$cod_cie10, "-")
  bd20122_1 <- data.frame(bd2012, do.call(rbind, bd20122_1))
  names(bd20122_1)
  names(bd20122_1)[15]='codcie10' #Verificar par reproducibilidad
  names(bd20122_1)[16]='defcie10' #Verificar par reproducibilidad
  ...
  for (i in seq_along(vector_raices_medic2)) {
    bd20122_1$dx_target <- if_else (bd20122_1$dx_target == TRUE,  bd20122_1$dx_target,
                                    str_detect(string =   bd20122_1$name_dx, pattern = vector_raices_medic2[i]))
  }
  ...
  bd2014 <- read_excel(file_paths_bd, sheet = "2014")
  names(bd2014) <- epitrix::clean_labels(names(bd2014))
  bd2014 <- bd2014 %>% mutate (name_drug = epitrix::clean_labels(med))

  bd2014$enf_cie10 <- NA
  bd2014$enf_cie10[bd2014$cod_cie10 %in% cods_cie10$codigo] <- 1

  bd2014$enf_cups <- NA
  bd2014$enf_cups[bd2014$cod_proc %in% cods_cups$codigo] <- 1
  bd2014$enf_cups[bd2014$cod_proc %in% names_cups] <- 1
  ...
  bd2015 <- read_excel(file_paths_bd, sheet = "2015")
  names(bd2015) <- epitrix::clean_labels(names(bd2015))
  bd2015 <- bd2015 %>% mutate (name_drug = epitrix::clean_labels(med))

  bd2015$enf_cie10 <- NA
  bd2015$enf_cie10[bd2015$cod_cie10 %in% cods_cie10$codigo] <- 1
  ...
  resultados <- data.frame("bd" = c("bd2011","bd2012","bd2014","bd2015"),
                             ...
                             "casos_dx_cups_atc" = c(sum(bd2011$medicamento_target, na.rm = TRUE), sum(bd20122_1$dx_target, na.rm = TRUE),
                                                     sum(bd2014$med_target, na.rm = TRUE), sum(datp4$med_target, na.rm = TRUE)),
                             ...
                             "casos_total" = c(sum(bd2011$casos, na.rm = TRUE), sum(datp2_1$casos, na.rm = TRUE),
